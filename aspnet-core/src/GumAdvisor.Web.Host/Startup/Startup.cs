...
using GumAdvisor.Identity;
using GumAdvisor.Web.Chat.SignalR;
using GumAdvisor.Web.Common;
using Swashbuckle.AspNetCore.Swagger;
using GumAdvisor.Web.IdentityServer;
using GumAdvisor.Web.Swagger;
using Stripe;
...
using GumAdvisor.Schemas;
using GumAdvisor.Web.HealthCheck;
using Newtonsoft.Json.Serialization;
using Owl.reCAPTCHA;
using HealthChecksUISettings = HealthChecks.UI.Configuration.Settings;
using Microsoft.AspNetCore.Server.Kestrel.Https;

namespace GumAdvisor.Web.Startup
{
    public class Startup
    {
        ...
        public IServiceProvider ConfigureServices(IServiceCollection services)
        {
            ...
            return services.AddAbp<GumAdvisorWebHostModule>(options =>
            {
        ...
        public void Configure(IApplicationBuilder app, IWebHostEnvironment env, ILoggerFactory loggerFactory)
        {
            ...
            if (WebConsts.SwaggerUiEnabled)
            {
                ...
                {
                    options.SwaggerEndpoint(_appConfiguration["App:SwaggerEndPoint"], "GumAdvisor API V1");
                    options.IndexStream = () => Assembly.GetExecutingAssembly()
                        .GetManifestResourceStream("GumAdvisor.Web.wwwroot.swagger.ui.index.html");
        ...
        private void ConfigureSwagger(IServiceCollection services)
        {
            ...
            {
                options.SwaggerDoc("v1", new OpenApiInfo() {Title = "GumAdvisor API", Version = "v1"});
                ...
                if (canShowSummaries)
                {
                    ...
                    var applicationXml = $"GumAdvisor.Application.xml";
                    var applicationXmlPath = Path.Combine(AppContext.BaseDirectory, applicationXml);
                    options.IncludeXmlComments(applicationXmlPath);

                    var webCoreXmlFile = $"GumAdvisor.Web.Core.xml";
